...
| 概念 | 谁控制 | 说明 |
|---|---|---|
| **指令(Prompt)** | 用户 | 当次具体任务,例如"用 superpowers 重构 auth 模块" |
| **Agent + Skill** | 工具厂商 + 用户 | Agent 是执行体,Skill 是 Agent 的可插拔扩展(方法论 / 工具集 / 领域知识) |
| **模型** | 模型厂商 | 底层能力,被 Agent 调用 |

**一句话总结**:模型换不动,Agent 可以换,指令必须自己写,Skill 可以自己装。
